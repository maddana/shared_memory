...
void checkfile()
{
        int f1=open("file_prod",O_RDONLY);
        if(f1<0)
        {
                perror("Open 1 failed");
                exit(11);
        }
        int f2=open("file_cons",O_RDONLY);
        if(f1<0)
        {
...
int main(int argc, char * argv[])
{
        ...
        int fd=open("file_cons",O_CREAT|O_WRONLY,S_IRWXU);
        if(fd<0)
        {
                perror("open failed");
                exit(5);
        ...
        while(flag==0)
        {
                ...
                int rr=semop(semid,sembuf_array,1);
                if(rr<0)
                {
